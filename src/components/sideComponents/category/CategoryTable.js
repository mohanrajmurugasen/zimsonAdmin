...
export default function CategoryTable({ search, setvals, vals }) {
  ...
  useEffect(() => {
    const dat = values.filter((country) => {
      return (
        country.categoryName.toLowerCase().indexOf(search.toLowerCase()) !== -1
      );
    });
  ...
  const handleClose2 = () => {
    authaxios.delete(`Categorys?id=${id}`).then((res) => {
      setOpen(false);
      setvals(!vals);
    });
  };
                ...
                <TableRow key={row.id}>
                  ...
                  <TableCell align="center">
                    <div className="action">
                      <div>
                        <EditIcon
                          onClick={() => handleOpen1(row.id, row.categoryName)}
                          color="primary"
                        />
                      </div>
                      <div>
