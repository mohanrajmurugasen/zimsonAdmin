...
export default function BrandTable({ search, setvals, vals }) {
  ...
  const handleClose2 = () => {
    authaxios.delete(`Brands?id=${id}`).then((res) => {
      setOpen(false);
      setvals(!vals);
    });
  };
  ...
  const filteredCountries = values.filter((country) => {
    return country.brandName.toLowerCase().indexOf(search.toLowerCase()) !== -1;
  });
  ...
  useEffect(() => {
    const dat = values.filter((country) => {
      return (
        country.brandName.toLowerCase().indexOf(search.toLowerCase()) !== -1 &&
      ...
      <TableContainer component={Paper}>
        <Table className={classes.table} aria-label="simple table">
          ...
          <TableBody>
            {filteredCountries &&
              filteredCountries
                .filter((name) => name.customerId === customerId)
                .map((row) => (
                  <TableRow key={row.id}>
                    ...
                    <TableCell align="center">
                      <div className="action">
                        <div>
                          <EditIcon
                            onClick={() => handleOpen1(row.id, row.brandName)}
                            color="primary"
                          />
                        </div>
                        <div>
          ...
          >
            <Fade style={{ width: "40%" }} in={open}>
              {modals === "delete" ? (
                ...
                <div className={classes.paper}>
                  <h2 id="transition-modal-title">Edit Brand</h2>
