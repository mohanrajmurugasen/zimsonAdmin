...
export default function AppUserTable({ search, setvals, vals }) {
  ...
  useEffect(() => {
    const dat = values.filter((country) => {
      return (
        country.username.toLowerCase().indexOf(search.toLowerCase()) !== -1
      );
    });
  ...
  const handleClose2 = () => {
    authaxios.delete(`AppUsers?id=${id}`).then((res) => {
      setOpen(false);
      setvals(!vals);
    });
  };
                  ...
                  <TableCell align="center">
                    <div className="action">
                      <div>
                        <EditIcon
                          onClick={() =>
                            handleOpen1(row.id, row.username, row.password)
                          }
                          color="primary"
                        />
                      </div>
          ...
          >
            <Fade style={{ width: "40%" }} in={open}>
              {modals === "delete" ? (
                ...
                <div className={classes.paper}>
                  <h2 id="transition-modal-title">Edit AppUser</h2>
