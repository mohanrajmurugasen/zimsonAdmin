...
export default function SiteTable({ search, setvals, vals }) {
  ...
  useEffect(() => {
    const dat = values.filter((country) => {
      return (
        country.siteName.toLowerCase().indexOf(search.toLowerCase()) !== -1
      );
    });
  ...
  const handleClose2 = () => {
    authaxios.delete(`Sites?id=${id}`).then((res) => {
      setvals(!vals);
      setOpen(false);
    });
  };
                ...
                <TableRow key={row.id}>
                  ...
                  <TableCell align="center">
                    <div className="action">
                      <div>
                        <EditIcon
                          onClick={() => handleOpen1(row.id, row.siteName)}
                          color="primary"
                        />
                      </div>
                      <div>
          ...
          >
            <Fade style={{ width: "40%" }} in={open}>
              {modals === "delete" ? (
                ...
                <div className={classes.paper}>
                  <h2 id="transition-modal-title">Edit Site</h2>
