...
export default function UserGroupTable({ search, setvals, vals }) {
  ...
  useEffect(() => {
    const dat = values.filter((country) => {
      return (
        country.userGroupName.toLowerCase().indexOf(search.toLowerCase()) !== -1
      );
    });
  ...
  const handleClose2 = () => {
    authaxios.delete(`UserGroups?id=${id}`).then((res) => {
      setOpen(false);
      setvals(!vals);
    });
  };
                ...
                <TableRow key={row.id}>
                  ...
                  <TableCell align="center">
                    <div className="action">
                      <div>
                        <EditIcon
                          onClick={() => handleOpen1(row.id, row.userGroupName)}
                          color="primary"
                        />
                      </div>
                      <div>
          ...
          >
            <Fade style={{ width: "40%" }} in={open}>
              {modals === "delete" ? (
                ...
                <div className={classes.paper}>
                  <h2 id="transition-modal-title">Edit UserGroup</h2>
