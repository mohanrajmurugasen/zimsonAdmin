...
export default function ProductTable({ search, setvals, vals }) {
  ...
  useEffect(() => {
    const dat = values.filter((country) => {
      return (
        country.productName.toLowerCase().indexOf(search.toLowerCase()) !== -1
      );
    });
  ...
  const handleClose2 = () => {
    authaxios.delete(`Products?id=${id}`).then((res) => {
      setvals(!vals);
      setOpen(false);
    });
  };
                ...
                <TableRow key={row.id}>
                  ...
                  <TableCell align="center">
                    <div className="action">
                      <div>
                        <EditIcon
                          onClick={() => handleOpen1(row.id, row.productName)}
                          color="primary"
                        />
                      </div>
                      <div>
