...
export default function TechnologyTable({ search, vals, setvals }) {
  ...
  useEffect(() => {
    const dat = values.filter((country) => {
      return (
        country.technologyName.toLowerCase().indexOf(search.toLowerCase()) !==
        -1
      );
    });
  ...
  const handleClose2 = () => {
    authaxios.delete(`Technologys?id=${id}`).then((res) => {
      setOpen(false);
      setvals(!vals);
    });
  };
                  ...
                  <TableCell align="center">
                    <div className="action">
                      <div>
                        <EditIcon
                          onClick={() =>
                            handleOpen1(row.id, row.technologyName)
                          }
                          color="primary"
                        />
                      </div>
          ...
          >
            <Fade style={{ width: "40%" }} in={open}>
              {modals === "delete" ? (
                ...
                <div className={classes.paper}>
                  <h2 id="transition-modal-title">Edit Technology</h2>
